...
public class NewDeviceAPIHybridProject extends AbstractHandler implements EgovDeviceAPIMenu {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (DeviceAPIIdeUtils.isMaven3Version() == false) {
			MessageDialog.openInformation(EgovDeviceAPIIdePlugin.getActiveWorkbenchWindow().getShell(), "Inform", "Selected function has not been installed. Maven3(M2E) is required.");
			return null;
		}

		if (DeviceAPIIdeUtils.isM2EToAndroid() == false) {
			MessageDialog.openInformation(EgovDeviceAPIIdePlugin.getActiveWorkbenchWindow().getShell(), "Inform", "Selected function has not been installed. M2E Configure for Android is required.");
			return null;
		}
		...
		if (wizardDesc == null) {
			MessageDialog.openInformation(EgovDeviceAPIIdePlugin.getActiveWorkbenchWindow().getShell(), "Inform", "Selected function has not been installed. eGovFrmae IDE is required.");
		} else {
