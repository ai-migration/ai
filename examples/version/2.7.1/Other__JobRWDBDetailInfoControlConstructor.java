 ...
 * </pre>
 */
public class JobRWDBDetailInfoControlConstructor extends
	 ...
	 * 
	 */
	public JobRWDBDetailInfoControlConstructor(
			...
			List<TypeCSqlKeyValueVo> preSavedSqlKeyValueList,
	...
	protected void createDetailInfoContents(Composite control,
		...
		} else if (DefaultJobRW.CUSTOMIZED_JDBC_CURSOR_ITEM_READER_TYPE
				.equals(resourceDetailType)) {
			createCustomizedJdbcCursorItemReaderControl(control);

		} else if (DefaultJobRW.SQL_PAGING_QUERY_JDBC_ITEM_READER_TYPE
				.equals(resourceDetailType)) {
			createSqlPagingQueryJdbcItemReaderControl(control);

		} else if (DefaultJobRW.SQL_PARAMETER_JDBC_BATCH_ITEM_WRITER_TYPE
				.equals(resourceDetailType)) {
			createSqlParameterJdbcBatchItemWriterControl(control);

		} else if (DefaultJobRW.ITEM_PREPARED_STATEMENT_JDBC_BATCH_ITEM_WRITER_TYPE
				.equals(resourceDetailType)) {
			createItemPreparedStatementJdbcBatchItemWriterControl(control);

		} else if (DefaultJobRW.EGOV_ITEM_PREPARED_STATEMENT_JDBC_BATCH_ITEM_WRITER_TYPE
				.equals(resourceDetailType)) {
			createEgovItemPreparedStatementJdbcBatchItemWriterControl(control);

		} else if (DefaultJobRW.EGOV_CUSTOMIZED_JDBC_BATCH_ITEM_WRITER_TYPE
				.equals(resourceDetailType)) {
		...
		text.addListener(SWT.Modify, new Listener() {
			...
			public void handleEvent(Event event) {
				String message = validation
						.validateIbatisRWAndGetErrorMessage();
				setMessage(message);
			}
		});
	}
	...
	private void createCustomizedJdbcCursorItemReaderControl(Composite control) {
		...
		setMessage(validation
				.validateCustomizedJdbcCursorItemReaderAndGetErrorMessage());
	}
	...
	private void createSqlPagingQueryJdbcItemReaderControl(Composite control) {
		...
		databaseTypeCombo.addListener(SWT.Modify, new Listener() {
			...
			public void handleEvent(Event event) {
				String message = validation
						.validateSqlPagingQueryJdbcItemReaderAndGetErrorMessage();
				setMessage(message);
			}
		});
	...
	private void createSqlParameterJdbcBatchItemWriterControl(Composite control) {
		...
		setMessage(validation
				.validateSqlParameterJdbcBatchItemWriterAndGetErrorMessage());
	}
	...
	private void createItemPreparedStatementJdbcBatchItemWriterControl(
			Composite control) {
		...
		setMessage(validation
				.validateItemPreparedStatementJdbcBatchItemWriterAndGetErrorMessage());
	}
	 ...
	 * @param text
	 */
	private void addSqlItemPreparedStatementJdbcBatchItemWriterItemListener(
			Text text) {
		text.addListener(SWT.Modify, new Listener() {
	...
	private void createEgovItemPreparedStatementJdbcBatchItemWriterControl(
			Composite control) {
		Text sqlInsertText = createSqlInsertControl(control);
		...
		setMessage(validation
				.validateEgovItemPreparedStatementJdbcBatchItemWriterAndGetErrorMessage());
	}
	 ...
	 * @param text
	 */
	private void addEgovItemPreparedStatementJdbcBatchItemWriterItemListener(
			Text text) {
		text.addListener(SWT.Modify, new Listener() {
	...
	private void createEgovCustomizedJdbcBatchItemWriterControl(
			Composite control) {
		Text sqlInsertText = createSqlInsertControl(control);
		...
		setMessage(validation
				.validateEgovCustomizedJdbcBatchItemWriterAndGetErrorMessage());
	}
	...
	private Text createIBatisStatementControl(Composite control) {
		Text iBatisStatementText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_IBATIS_STATEMENT_LABEL);
		setTextHorizontalFullAndSpan(iBatisStatementText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.IBATIS_STATEMENT,
				iBatisStatementText);
		setInitialValueToText(JobRWDetailInfoItem.IBATIS_STATEMENT,
				iBatisStatementText);
	...
	private Text createConfigurationFileControl(Composite control) {
		Text configurationFileText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_CONFIGURATION_FILE_LABEL);
		addSaveValueToContextListener(JobRWDetailInfoItem.CONFIGURATION_FILE,
				configurationFileText);
		setInitialValueToText(JobRWDetailInfoItem.CONFIGURATION_FILE,
				configurationFileText);

		Button browseButton = createBrowseButton(control);
		browseButton.addListener(SWT.Selection,
				getConfigurationButtonListener(control, configurationFileText));
	...
	private Text createSqlControl(Composite control) {
		Text sqlText = createLabelText(control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_LABEL);
	...
	private Text createRowMapperControl(Composite control) {
		Text rowMapperText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_ROW_MAPPER_LABEL);
		addSaveValueToContextListener(JobRWDetailInfoItem.ROW_MAPPER,
				rowMapperText);
		...
		button.addListener(SWT.Selection,
				getClassSearchBrowseButtonListener(rowMapperText));
	...
	private Text createPageSizeControl(Composite control) {
		Text pageSizeText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_PAGE_SIZE_LABEL);
		setTextHorizontalFullAndSpan(pageSizeText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.PAGE_SIZE,
				pageSizeText);
	...
	private Text createSqlSortColumnControl(Composite control) {
		Text sqlSortColumn = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_SORT_COLUMN_LABEL);
		sqlSortColumn
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_SORT_COLUMN_EXAMPLE);
		setTextHorizontalFullAndSpan(sqlSortColumn, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.SQL_SORT_COLUMN,
				sqlSortColumn);
		setInitialValueToText(JobRWDetailInfoItem.SQL_SORT_COLUMN,
				sqlSortColumn);
	...
	private Text createSqlSelectControl(Composite control) {
		Text sqlSelectText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_SELECT_LABEL);
		sqlSelectText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_SELECT_EXAMPLE);
		setTextHorizontalFullAndSpan(sqlSelectText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.SQL_SELECT,
				sqlSelectText);
	...
	private Text createSqlFromControl(Composite control) {
		Text sqlFromText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_FROM_LABEL);
		sqlFromText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_FROM_EXAMPLE);
	...
	private Text createSqlWhereControl(Composite control) {
		Text sqlWhereText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_WHERE_LABEL);
		sqlWhereText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_WHERE_EXAMPLE);
		setTextHorizontalFullAndSpan(sqlWhereText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.SQL_WHERE,
				sqlWhereText);
	...
	private Text createSqlUpdateControl(Composite control) {
		Text sqlUpdateText = createLabelText(control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_LABEL);
		sqlUpdateText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_UPDATE_EXAMPLE);
		setTextHorizontalFullAndSpan(sqlUpdateText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.SQL_UPDATE,
				sqlUpdateText);
	...
	private Text createSqlInsertControl(Composite control) {
		Text sqlInsertText = createLabelText(control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_LABEL);
		sqlInsertText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_INSERT_EXAMPLE);
		setTextHorizontalFullAndSpan(sqlInsertText, 2);
		addSaveValueToContextListener(JobRWDetailInfoItem.SQL_INSERT,
				sqlInsertText);
	...
	private Text createParamsControl(Composite control) {
		Text paramsText = createLabelText(control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_PARAMS_LABEL);
		paramsText
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_PARAMS_EXAMPLE);
	...
	private Text createRowSetterControl(Composite control) {
		Text rowSetterText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_ROW_SETTER_LABEL);
		addSaveValueToContextListener(JobRWDetailInfoItem.ROW_SETTER,
				rowSetterText);
		setInitialValueToText(JobRWDetailInfoItem.ROW_SETTER, rowSetterText);

		Button button = createBrowseButton(control);
		button.addListener(SWT.Selection,
				getClassSearchBrowseButtonListener(rowSetterText));
	...
	private TableViewer createTableViewer(Composite control) {
		...
		BatchTableColumn keyColumn = new BatchTableColumn(
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_TABLE_COLUMN_KEY,
				150);
		BatchTableColumn valueColumn = new BatchTableColumn(
				BatchMessages.JobRWDBDetailInfoControlConstructor_SQL_TABLE_COLUMN_VALUE,
				180);
	 ...
	 * @param tableViewer
	 */
	private void createTableViewerButtonControl(Composite control,
	 ...
	 * @param tableViewer
	 */
	private void createAddButton(final Composite control,
			final TableViewer tableViewer) {
		Button addButton = new Button(control, SWT.PUSH);
		addButton
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_ADD_BUTTON);
		addButton.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		addButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				SqlKeyValueTableDialog dialog = new SqlKeyValueTableDialog(
						control.getShell());
	 ...
	 * @param tableViewer
	 */
	private void createRemoveButton(Composite control,
			final TableViewer tableViewer) {
		Button removeButton = new Button(control, SWT.PUSH);
		removeButton
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_REMOVE_BUTTON);
		...
		removeButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				if (!selection.isEmpty()) {
					TypeCSqlKeyValueVo vo = (TypeCSqlKeyValueVo) selection
							.getFirstElement();
	...
	private Combo createDatabaseType(Composite control) {
		...
		databaseTypeLabel
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_DATABASE_TYPE_LABEL);

		GridData gData = new GridData(GridData.FILL_HORIZONTAL);
		gData.horizontalSpan = 2;

		final Combo databaseTypeCombo = new Combo(control, SWT.BORDER
				| SWT.READ_ONLY);

		// 나중에 이 STRING이 어디서 사용되나 확인하고 어디에 static public 으로 정의해야 하는지 확인
		databaseTypeCombo.setItems(new String[] { "Oracle", "Hsql", "mySql",
				"Altibase", "Tibero" });
		databaseTypeCombo.setLayoutData(gData);
		addSaveValueToContextListener(JobRWDetailInfoItem.DATABASE_TYPE,
				databaseTypeCombo);
		setInitialValueToCombo(JobRWDetailInfoItem.DATABASE_TYPE,
				databaseTypeCombo);
	...
	private Text createDatasourceBeanIDText(final Composite control) {
		final Text datasourceBeanIDText = createLabelText(
				control,
				BatchMessages.JobRWDBDetailInfoControlConstructor_DATABASE_BEAN_ID_LABEL);
		addSaveValueToContextListener(JobRWDetailInfoItem.DATASOURCE_BEAN_ID,
				datasourceBeanIDText);
		setInitialValueToText(JobRWDetailInfoItem.DATASOURCE_BEAN_ID,
				datasourceBeanIDText);

		Button datasourceBrowseButton = new Button(control, SWT.PUSH);
		datasourceBrowseButton
				.setText(BatchMessages.JobRWDBDetailInfoControlConstructor_BROWSE_BUTTON);
		datasourceBrowseButton.addListener(SWT.Selection, new Listener() {

			public void handleEvent(Event event) {

				String selectedBeanID = "";
				GetDatasourceBeanIDDialog dialog = new GetDatasourceBeanIDDialog(
						control.getShell(), context, selectedBeanID);
 ...
 * 
 */
class KeyValueTableLabelProvider extends LabelProvider implements
		ITableLabelProvider {
