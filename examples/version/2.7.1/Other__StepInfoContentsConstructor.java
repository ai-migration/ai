...
public class StepInfoContentsConstructor extends InfoContentsConstructor {
	 ...
	 * 
	 */
	public StepInfoContentsConstructor(WizardPage currentPage,
	...
	public void createStepInfoContents(JobVo jobVo, StepVo stepVo) {
		...
		commitIntervalString = ConvertIntegerToString(tempStepVo
				.getCommitInterval());
	...
	private void createJobRWAndCommitIntervalGroup() {
		...
		itemModeGroup
				.setText(BatchMessages.StepInfoContentsConstructor_ITEM_CONFIGURATION_GROUP);
	...
	private void createJobRWControl(Composite control) {
		...
		readerLabel
				.setText(BatchMessages.StepInfoContentsConstructor_READER_LABEL);
		...
		writerLabel
				.setText(BatchMessages.StepInfoContentsConstructor_WRITER_LABEL);
	...
	private Text createJobReaderText(Composite subControl) {
		...
		button.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				JobRWDialog dialog = new JobReaderDialog(
						infoControl.getShell(), invalidBatchXMLFileBeanIDList,
						pageBeanIDs, tempStepVo, context);
				if (dialog.open() == org.eclipse.jface.window.Window.OK) {
					...
					tempStepVo.setJobReadersqlKeyValueVo(dialog
							.getSqlKeyValueVo());
				}
			}
		});
	...
	private Text createJobWriterText(Composite subControl) {
		...
		button.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				JobRWDialog dialog = new JobWriterDialog(
						infoControl.getShell(), invalidBatchXMLFileBeanIDList,
						pageBeanIDs, tempStepVo, context);
				if (dialog.open() == org.eclipse.jface.window.Window.OK) {
					pageBeanIDs.removeBeanID(text.getText());
	...
	private String getJobReaderFullName(StepVo stepVo) {
		return getJobRWFullName(stepVo.getJobName(), stepVo.getName(),
				stepVo.getJobReaderInfo());
	}
	...
	private void creatCommitIntervalControl(Composite control) {
		...
		commitIntervalLabel
				.setText(BatchMessages.StepInfoContentsConstructor_COMMIT_INTERVAL_LABEL);
		...
		commitIntervalField
				.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		commitIntervalField
				.setText(returnEmptyStringIfNullOrConvertToString(commitinterval));
	...
	private TableViewer createStepListenerTabItem(TabFolder subControl) {
		...
		TableViewer tableViewer = createListenerTableViewer(listenerControl,
				listenerInput);
	 ...
	 * @return
	 */
	private TableViewer createListenerTableViewer(Composite control,
			ListenerInfo[] listenerInput) {
		final TableViewer tableViewer = new TableViewer(control, SWT.BORDER
	...
	private BatchTableColumn[] createListenerTableColumns() {
		...
		columns.add(new BatchTableColumn(
				BatchMessages.StepInfoContentsConstructor_LISTENER_TABLE_COLUMN_NAME,
				180, SWT.LEFT));
		columns.add(new BatchTableColumn(
				BatchMessages.StepInfoContentsConstructor_LISTENER_TABLE_COLUMN_CLASS,
				180, SWT.LEFT));
	 ...
	 * @param tableViewer
	 */
	private void createChunkListenerTableButtons(Composite control,
		...
		removeButton
				.setText(BatchMessages.StepInfoContentsConstructor_REMOVE_BUTTON);
		...
		addButton.addListener(SWT.Selection,
				chunkListenerTableListener(tableViewer, removeButton));
		removeButton.addListener(SWT.Selection,
				chunkListenerRemoveButtonListener(tableViewer, removeButton));
	}
	 ...
	 * @param tableViewer
	 */
	private void createStepListenerTableButtons(Composite control,
			TableViewer tableViewer) {
		Composite buttonControl = new Composite(control, SWT.None);
		buttonControl.setLayout(new GridLayout());
		buttonControl.setLayoutData(new GridData());
		...
		removeButton.setEnabled(false);
		removeButton.addListener(SWT.Selection,
				stepListenerRemoveButtonListener(tableViewer, removeButton));
		...
		addButton.addListener(SWT.Selection,
				stepListenerTableListener(tableViewer, removeButton));
	}
	 ...
	 * @return
	 */
	private Listener stepListenerRemoveButtonListener(
			final TableViewer tableViewer, final Button removeButton) {
		return new Listener() {
	 ...
	 * @return
	 */
	private Listener chunkListenerRemoveButtonListener(
			final TableViewer tableViewer, final Button removeButton) {
		return new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (!NullUtil.isNull(selection)) {
					...
					if (!NullUtil.isEmpty(items)) {
						for (int i = 0; i < items.length; i++) {
							chunkData[i] = (ChunkListenerInfo) items[i]
									.getData();
						}
					}
	 ...
	 * @return
	 */
	private Listener stepListenerTableListener(final TableViewer tableviewer,
		...
		return new Listener() {
			public void handleEvent(Event event) {
				...
				ListenerDialog dialog = new StepListenerDialog(
						infoControl.getShell(), listenerNames,
						invalidBatchXMLFileBeanIDList, pageBeanIDs);
				if (dialog.open() == org.eclipse.jface.window.Window.OK) {
	 ...
	 * @return
	 */
	private Listener chunkListenerTableListener(final TableViewer tableviewer,
			final Button removeButton) {
		return new Listener() {
			public void handleEvent(Event event) {
				TableItem[] items = tableviewer.getTable().getItems();
				...
				ListenerDialog dialog = new ChunkListenerDialog(
						infoControl.getShell(), listenerNames,
						invalidBatchXMLFileBeanIDList, pageBeanIDs);
				if (dialog.open() == org.eclipse.jface.window.Window.OK) {
					ListenerInfo[] data = dialog.getInfo();
	...
	private void createSharedValuesTabItem(TabFolder subControl, JobVo jobVo) {
		...
		sharedValueTableViewer = createSharedValueTableViewer(
				sharedValuesControl, jobVo);
	 ...
	 * @return
	 */
	private TableViewer createSharedValueTableViewer(Composite control,
			JobVo jobVo) {
		sharedValueTableViewer = new TableViewer(control, SWT.BORDER
				| SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
		...
		sharedValueTableViewer.getControl().setLayoutData(
				new GridData(GridData.FILL_BOTH));
		sharedValueTableViewer
				.setLabelProvider(new SharedValuesTableProvider());
	...
	private BatchTableColumn[] createSharedValueTableColumns() {
		ArrayList<BatchTableColumn> columns = new ArrayList<BatchTableColumn>();
		columns.add(new BatchTableColumn(
				BatchMessages.StepInfoContentsConstructor_SHARED_VALUES_TABLE_COLUMN_KEY,
				250, SWT.LEFT));
		columns.add(new BatchTableColumn(
				BatchMessages.StepInfoContentsConstructor_SHARED_VALUES_TABLE_COLUMN_STEP_ID,
				120, SWT.LEFT));
	...
	private void createSharedValueTableViewerButtons(Composite control) {
		...
		Button addButton = new Button(buttonControl, SWT.PUSH);
		addButton
				.setText(BatchMessages.StepInfoContentsConstructor_SHARED_VALUES_TABLE_COLUMN_);
		addButton.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		addButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				SharedValueDialog dialog = new SharedValueDialog(infoControl
						.getShell(), list, tempStepVo,
						invalidBatchXMLFileBeanIDList, pageBeanIDs);
		...
		sharedValueTableViewer.getTable().addListener(SWT.Selection,
				new Listener() {
					public void handleEvent(Event event) {
						removeButton.setEnabled(true);
					}
	 ...
	 * @return
	 */
	private SharedValueVo[] getSharedValueVoFromTableViewer(
			TableViewer tableviewer) {
		TableItem[] items = tableviewer.getTable().getItems();
	...
	private void createStepIDAndNext() {
		...
		stepIdField.setText(StringUtil.returnEmptyStringIfNull(tempStepVo
				.getName()));
		...
		activeNext
				.setText(BatchMessages.StepInfoContentsConstructor_NEXT_CHECK_BOX);
	...
	public BatchXMLFileBeanIDList getBatchJobsInvalidBeanList() {
		...
		BatchJobBeanIDList jobBeanIDList = invalidList
				.getJobBeanIDList(tempStepVo.getJobName());
	...
	public String[] getStepAndDecisionNameListWithoutThisStepVoName(JobVo jobVo) {
		...
		String sdVoName = StringUtil.returnEmptyStringIfNull(tempStepVo
				.getName());
	...
	private void createStepTypeOptionControl() {
		...
		partitionTypeControl.setLayoutData(new GridData(
	...
	private void createStepTypeControl() {
		...
		stepTypeGroup
				.setText(BatchMessages.StepInfoContentsConstructor_STEP_TYPE_GROUP);

		Button stepTypeNormal = new Button(stepTypeGroup, SWT.RADIO);
		stepTypeNormal.setLayoutData(new GridData());
		stepTypeNormal
				.setText(BatchMessages.StepInfoContentsConstructor_NORMAL_RADIO_BUTTON);
		...
		stepTypePartition
				.setText(BatchMessages.StepInfoContentsConstructor_PARTITION_RADIO_BUTTON);
		stepTypePartition.addListener(SWT.Selection,
				selectPartitionerTypeListener);
	...
	private void createPartitionTypeOption() {
		...
		Composite partitionTypeOptionControl = new Composite(
				partitionTypeControl, SWT.None);
		partitionTypeOptionControl.setLayout(new GridLayout(4, false));
		partitionTypeOptionControl.setLayoutData(new GridData(
				GridData.FILL_HORIZONTAL));
	...
	private void createSubstepControl(Composite control) {
		...
		subStepIdLabel
				.setText(BatchMessages.StepInfoContentsConstructor_SUB_STEP_ID_LABEL);
	...
	private void createGridSizeControl(Composite control) {
		...
		gridSizeLabel
				.setText(BatchMessages.StepInfoContentsConstructor_GRID_SIZE_LABEL);
		...
		gridSizeText
				.setText(returnEmptyStringIfNullOrConvertToString(tempStepVo
						.getGridSize()));
	...
	private void createResourceControl(Composite control) {
		...
		ResourceLabel
				.setText(BatchMessages.StepInfoContentsConstructor_RESOURCE_LABEL);
		...
		resourceText.setText(StringUtil.returnEmptyStringIfNull(tempStepVo
				.getResource()));
		...
		resourceButton
				.setText(BatchMessages.StepInfoContentsConstructor_BROWSE_BUTTON);
		resourceButton.setLayoutData(resourceGData);
		resourceButton.addListener(SWT.Selection,
				getResourceButtonListener(resourceText));
		...
		exampleLabel
				.setText(BatchMessages.StepInfoContentsConstructor_RESOURCE_EXAMPLE);
		...
		return new Listener() {
			...
			public void handleEvent(Event event) {
				FileDialog fileDialog = new FileDialog(
						partitionTypeControl.getShell(), SWT.OPEN);
				fileDialog
						.setText(BatchMessages.StepInfoContentsConstructor_RESOURCE_BUTTON_DIALOG_TITLE);
				fileDialog.setFilterPath(context.getProject().getLocation()
						.toOSString());
				...
				if (NullUtil.isEmpty(selectedResource)) {
					selectedResource = StringUtil
							.returnEmptyStringIfNull(resourceText.getText());
				} else {
	 ...
	 * @return
	 */
	public SharedValueVo[] updateStepIDToSharedValueVoAndGetList(
