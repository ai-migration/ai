...
public class ModelListMaker implements ListMaker {
	...
	private void makeClassesList(String parentPackageName,
		...
		for (int i = 0; i < package_.getPackagedElements().size(); i++) {
			...
			if (child instanceof org.eclipse.uml2.uml.Class) {
				this.target
						.put(packageName
								+ "."
								+ ((org.eclipse.uml2.uml.Class) child)
										.getName(), child);
			}
			if (child instanceof org.eclipse.uml2.uml.Interface) {
				this.target.put(packageName + "."
						+ ((org.eclipse.uml2.uml.Interface) child).getName(),
						child);
			}
			if (!(child instanceof org.eclipse.uml2.uml.PrimitiveType)
					&& child instanceof org.eclipse.uml2.uml.DataType) {
				this.target.put(packageName + "."
						+ ((org.eclipse.uml2.uml.DataType) child).getName(),
						child);
			}
			if (child instanceof org.eclipse.uml2.uml.Package) {
				makeClassesList(packageName,
