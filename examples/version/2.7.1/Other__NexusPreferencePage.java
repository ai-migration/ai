 ...
 * </pre>
 */
public class NexusPreferencePage extends PreferencePage implements
		...
		if (nexusCount > 0)
			for (int i = 0; i < nexusCount; i++) {
				...
				PrefrencePropertyUtil.savePreferences(
						EgovConfMngtPlugin.getDefault(), nexusInfo);
			}
		else {
			PrefrencePropertyUtil.savePreferences(
					EgovConfMngtPlugin.getDefault(), new NexusInfo());
		}
	...
	protected Control createContents(Composite composite) {
		...
		// Create SashForm
//		SashForm sashFormSub = new SashForm(composite, SWT.VERTICAL);
		GridData gData = new GridData(GridData.FILL_BOTH);
		gData.heightHint = 650;
		gData.widthHint = 460;
//		sashFormSub.setLayoutData(gData);
//
//		Composite container = new Composite(sashFormSub, SWT.NONE);
...
//		container.setLayout(layout);
		...
		// Create table
		tableViewer = new TableViewer(innerContainer, SWT.BORDER | SWT.H_SCROLL
...
//		GridLayoutFactory.fillDefaults().generateLayout(innerContainer);
			...
			public void handleEvent(Event event) {
				...
				if (itemCnt > 0)
					for (int i = 0; i < itemCnt; i++) {
						NexusInfo nexusInfo = (NexusInfo) tableViewer
								.getTable().getItem(i).getData();
						nexusIdList.add(nexusInfo.getNexusId());
					}

				NexusDialog nexusInfoDialog = new NexusDialog(getControl()
						.getShell(), null, null, true, true, true, nexusIdList);

				if (nexusInfoDialog.open() == Window.OK) {
//					boolean isEqual = false;
//					int nexusCnt = EgovConfMngtPlugin.getDefault()
//							.getPreferenceStore().getInt("nexusCount");
//					if (nexusCnt > 0) {
//						for (int i = 0; i < nexusCnt; i++) {
//							if (nexusInfoDialog.getNexusId().equals(
//									tableViewer.getTable().getItem(i)
//											.getText(i))) {
//								isEqual = true;
//							}
//						}
...
//					if (isEqual) {
//
//						setErrorMessage("해당 ID는 이미 설치되어 있습니다.");
//						StatusManager.getManager().handle(
//								new Status(IStatus.CANCEL,
//										EgovConfMngtPlugin.PLUGIN_ID,
//										"해당 ID는 이미 설치되어 있습니다."));
//
//					} else {
						nexusInfo = new NexusInfo(nexusInfoDialog.getNexusId(),
								nexusInfoDialog.getNexusUrl(), nexusInfoDialog
										.isReleaseButtonPressed(),
								nexusInfoDialog.isSnapshotsButtonPressed());
			...
			public void handleEvent(Event event) {
				...
				if (!NullUtil.isNull(selection) && selection.size() > 0) {
					...
					if (object instanceof NexusInfo) {
						...
						NexusDialog nexusInfoDialog = new NexusDialog(
								getControl().getShell(),
								nexusInfo.getNexusId(),
								nexusInfo.getNexusUrl(), nexusInfo
										.getIsRealeaseSelected(), nexusInfo
										.getIsSnapshotsSelected(), false,
								nexusIdList);

						if (nexusInfoDialog.open() == Window.OK) {
							nexusInfo.setNexusId(nexusInfoDialog.getNexusId());
							nexusInfo.setNexusUrl(nexusInfoDialog.getNexusUrl());
							nexusInfo.setIsRealeaseSelected(nexusInfoDialog
									.isReleaseButtonPressed());
							nexusInfo.setIsSnapshotsSelected(nexusInfoDialog
