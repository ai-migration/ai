	...
	public boolean performFinish() {
        ...
        Job job = new Job(EgovModelCodeGenPlugin.getDefault().getResourceString("import.job.name")) {
			...
			protected IStatus run(IProgressMonitor monitor) {
		        ...
		        monitor.beginTask(EgovModelCodeGenPlugin.getDefault().getResourceString(
				"import.job.task.name"), objects.size() * 2 + 1);
		       ...
		       /* Iterator keys = stereoName.keySet().iterator();
				...
				for (int i = 0; i < objects.size(); i++) {
					...
					if (object instanceof NamedElement) {
						...
						monitor.subTask(EgovModelCodeGenPlugin.getDefault()
								.getResourceString(
